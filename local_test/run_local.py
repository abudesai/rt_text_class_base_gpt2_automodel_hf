    ...
    dir_tree = {
        "ml_vol": {
            "inputs": {
                "data_config": None,
                "data": {
                    "training": {"textClassificationBaseMainInput": None},
                    "testing": {"textClassificationBaseMainInput": None},
                },
            },
            "model": {
                "model_config": None,
...
def train_and_save_algo():
    ...
    preprocessor, model = model_trainer.get_trained_model(
        train_data, data_schema, hyper_parameters, model_artifacts_path
    )
...
def score(test_key, predictions, data_schema):
    ...
    id_field = data_schema["inputDatasets"]["textClassificationBaseMainInput"][
        "idField"
    ]
    target_field = data_schema["inputDatasets"]["textClassificationBaseMainInput"][
        "targetField"
    ]
    ...
    if len(class_names) == 2:
        ...
        auc = roc_auc_score(
            mapped_classes_true, predictions[class_names].values[:, minority_class_idx]
        )
    else:
        auc = roc_auc_score(
            mapped_classes_true,
